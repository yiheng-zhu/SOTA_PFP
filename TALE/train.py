...
def cut(a, maxlen):
    ...
    for i in a:
        if len(i) > maxlen:
            start = np.random.randint(len(i) - maxlen + 1)
...
class HMC_models(object):
    ...
    def regular_loss(self, pred_out):
        hparams = self.hparams
        label_regular = np.loadtxt(hparams['data_path'] + "/parent_index").astype(np.int64)
        ...
        with tf.Session() as sess:
            ...
            for epoch in range(hparams['epochs']):
                ...
                for ite in range(iterations):
                    ...
                    train_loss, _, regular_loss = sess.run([holder_list[2], train_op, holder_list[4]],
                                                           {holder_list[0]: x, holder_list[1]: y})

                    sepoch_train_loss += train_loss
                    print("iteration %d/%d totaltrain_loss: %.3f regular loss %.3f" % (
                    ite + 1, iterations, train_loss, regular_loss))
                ...
                for ite in range(iterations):
                    x = cut(val_x[ite * batch_size: (ite + 1) * batch_size], hparams['MAXLEN'])
                ...
                print("epoch %d, train_loss: %.3f val_loss: %.3f" % (
                    epoch + resume_epoch + 1, sepoch_train_loss, sepoch_val_loss))
                ...
                current_save_path = hparams['save_path'] + "/" + str(epoch + 1) + "/"
                if (os.path.exists(current_save_path) == False):
                    os.makedirs(current_save_path)

                # save_model..................
                saver.save(sess, current_save_path + "/ckpt_" + "epoch_" + \
                           str(epoch + resume_epoch + 1))
                with open(current_save_path + "/ckpt_" + "epoch_" + \
    ...
    def data_load(self, path):
        ...
        train_seq = self.read_sequence(path + "/train_sequence.fasta")
        train_label = self.read_label(path + "/train_label_one_hot")


        val_seq = self.read_sequence(path + "/test_sequence.fasta")
        val_label = self.read_label(path + "/test_label_one_hot")
        ...
        for i in range(len(train_seq)):
            train_X.append(amino_acid.to_int(train_seq[i], self.hparams))

        for i in range(len(val_seq)):
            val_X.append(amino_acid.to_int(val_seq[i], self.hparams))
